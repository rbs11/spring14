...
static void quarentineLogEntiresForSegment(RVMINFO* currNode,
		const char* segment, void* writeTo) {
	if (currNode && segment) {
		...
		sprintf(tempLogFileName, "%s/%s%s", currNode->directory,
				LOGNAME_PREFIX, "tmp");
					...
					if (!startsWith(TRANSEND, line)) {
						if (startsWith(SEGNAMESTR, line)) {
							if (!strcmp(line + strlen(SEGNAMESTR), segment)) {
								...
								unsigned long segOffset = atol(
										line + strlen(SEGOFFSET));
								readLineFromFile(flogFile, line, sizeof(line));
								unsigned long noOfBytesToWrite = atol(
										line + strlen(DIFFSIZE));
								writeBytesToMemory(
										flogFile,
										(writeTo ? (writeTo + segOffset) : NULL),
										noOfBytesToWrite);
							} else {
								...
								unsigned long noOfBytesToWrite = atol(
										line + strlen(DIFFSIZE));
								writeBytesToFile(flogFile, ftempLogFile,
										noOfBytesToWrite);
							}
						} else {
							if (!transactionHeaderWritten) {
...
void* getMappedTruncateRegion(SEGTRUNCATE* freeSegments, char *segmentname,
		char *directory) {
	...
	if (freeSegments && !freeSegments->mappedBase) {
		...
		if (fd > 0) {
			freeSegments->mappedBase = mmap(NULL, freeSegments->size,
...
static void removeSegbaseFromTransaction(TRANSINFO* currInfo,
		SEGINFO *mappedBase) {
...
rvm_t rvm_init(const char *directory) {
	...
	if (directory) {
		...
		toInsert->logFilePath = combinePaths(toInsert->directory,
				LOGNAME_PREFIX);
		...
		if (!directoryExists) {
			if (mkdir(directory, 0777)) {
				fprintf(stderr,
						"rvm_init: Problem occured while trying to create the directory provided\n");
				abort();
			}
		}
...
void *rvm_map(rvm_t rvm, const char *segname, int size_to_create) {
	void *to_ret = NULL;
	if (!segname || startsWith(LOGNAME_PREFIX, segname) || startsWith(
			SEGINFO_FILE, segname)) {
		fprintf(
				stderr,
				"rvm_map:Invalid segment file name, lrvm uses these to store its own metadata\n");
	} else {
		...
		if (currNode && !isSegmentMapped(currNode, segname)) {
			if (isSegmentExists(currNode->directory, segname, &currSegSize)) {
				if (currSegSize > size_to_create) {
					...
					createNewSegment(currNode->directory, segname,
							size_to_create, 1);
				}

			} else {
				createNewSegment(currNode->directory, segname, size_to_create,
						0);
			}
			...
			char* targetSegmentFile =
					combinePaths(currNode->directory, segname);
			int fd = open(targetSegmentFile, O_RDWR);
			if (fd > 0) {
				void* copiedContents = mmap(NULL, size_to_create, PROT_WRITE,
						MAP_SHARED, fd, 0);
				/*if (currNode->logFileFD) {
				 fclose(currNode->logFileFD);
				 currNode->logFileFD = NULL;
				 ...
				 copyFile(tempLogFileName, currNode->logFilePath);
				 deleteFile(tempLogFileName);*/
				quarentineLogEntiresForSegment(currNode, segname,
						copiedContents);
...
void rvm_destroy(rvm_t rvm, const char *segname) {
	...
	if (currNode && segname) {
		if (isSegmentMapped(currNode, segname)) {
			fprintf(
					stderr,
					"rvm_destroy:Segment:%s is currently mapped and cannot be destroyed\n",
					segname);
			abort();
		}
	...
	if (currNode && segbase) {
		...
		if (targetSegInfo) {
			if (targetSegInfo->isInTransaction) {
				TRANSINFO* targetTrans = getTransactionByMappedBase(
						targetSegInfo);
				while (targetTrans) {
...
trans_t rvm_begin_trans(rvm_t rvm, int numsegs, void **segbases) {
	RVMINFO* currNode = getRVMINFONode(rvm);
	int i = 0;
	if (currNode && numsegs > 0 && segbases && areSegsFree(currNode, numsegs,
			segbases)) {
		...
		currTrans->mappedSegments = (SEGINFO**) malloc(
				numsegs * sizeof(SEGINFO*));
		for (i = 0; i < numsegs; i++) {
			SEGINFO* currSegNode =
					getSegmentByMappedBase(currNode, segbases[i]);
...
void rvm_about_to_modify(trans_t tid, void *segbase, int offset, int size) {
	...
	if (segbase && currTransInfo && isSegmentValid(currTransInfo, segbase,
			offset, size) && !isModificationRecordPresent(currTransInfo,
			segbase, offset, size)) {
...
void rvm_commit_trans(trans_t tid) {
	...
	if (currTransInfo) {
		if (!currTransInfo->targetRVM->logFileFD) {
			currTransInfo->targetRVM->logFileFD = fopen(
					currTransInfo->targetRVM->logFilePath, "a");
		}
		if (currTransInfo->modificationRecords) {
			...
			while (currModRec) {
				SEGINFO* currSegInfo = getSegmentByMappedBase(
						currTransInfo->targetRVM, currModRec->segmentUserBase);
				fprintf(logFD, "%s%s\n", SEGNAMESTR, currSegInfo->name);
				sprintf(line, "%s%lu", SEGOFFSET, currModRec->offset);
				fprintf(logFD, "%s\n", line);
				sprintf(line, "%s%lu", DIFFSIZE, currModRec->size);
				fprintf(logFD, "%s\n", line);
				writeBytesToFileFromMemory(logFD,
						currModRec->segmentUserBase + currModRec->offset,
						currModRec->size);
...
void rvm_abort_trans(trans_t tid) {
	...
	if (currTransInfo) {
		...
		while (currRecord) {
			...
			memcpy(currRecord->segmentUserBase + currRecord->offset,
					currRecord->backupRegion, currRecord->size);
			...
			if (fd) {
				...
				while (readLineFromFile(fd, line, sizeof(line))) {
					if (startsWith(SEGNAMESTR, line)) {
						...
						SEGTRUNCATE* toAppend = (SEGTRUNCATE*) malloc(
								sizeof(SEGTRUNCATE));
			...
			while (readLineFromFile(flogFile, line, sizeof(line))) {
				if (startsWith(TRANSSTART, line)) {
					...
					while (readLineFromFile(flogFile, line, sizeof(line))) {
						if (!startsWith(TRANSEND, line)) {
							if (startsWith(SEGNAMESTR, line)) {
								strcpy(currSegmentName,
										line + strlen(SEGNAMESTR));
								if (1/*!isSegmentMapped(currNode, currSegmentName)*/) {
									readLineFromFile(flogFile, line,
											sizeof(line));
									unsigned long segOffset = atol(
											line + strlen(SEGOFFSET));
									readLineFromFile(flogFile, line,
