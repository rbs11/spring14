...
// map a segment from disk into memory. If the segment does not already exist, then create it and give it size size_to_create. If the segment exists but is shorter than size_to_create, then extend it until it is long enough. It is an error to try to map the same segment twice.
{
	...
	struct segments *temp_seg = (struct segments *) malloc(
	...
	if (first == 0) {
		...
		while (segments != NULL) {
			if (!strcmp(segments->segname, segname)) {
				allocated = 1;
				segments->segment = realloc(segments->segment,
						sizeof(char) * size_to_create);
				return segments->segment;
				//break;
			}
...
void rvm_unmap(rvm_t rvm, void *segbase) // unmap a segment from memory.
{
	...
	struct segments *temp_seg = (struct segments *) malloc(
			sizeof(struct segments));
	struct segments *remove =
			(struct segments*) malloc(sizeof(struct segments));
	temp_seg = headnode;
	if (temp_seg == NULL)
		exit(0);
...
trans_t rvm_begin_trans(rvm_t rvm, int numsegs, void **segbases) // begin a transaction that will modify the segments listed in segbases. If any of the specified segments is already being modified by a transaction, then the call should fail and return (trans_t) -1.
{
	...
	struct segments *remove =
			(struct segments*) malloc(sizeof(struct segments));
	struct segments *temp_remove = (struct segments *) malloc(
			sizeof(struct segments));
	remove = headnode;
	/*printf("coming hre\n");
	 while(remove!=NULL){
		...
		while (temp_seg != NULL) {
			...
			if (k <= numsegs)
				for (m = 0; m < ofset; m++) {
					...
					if (!strcmp(temp_seg->segment + temp_seg->offsets[m],
							(char*) segbases[k] + temp_seg->offsets[m])
							&& temp_seg->tidcount == 0) {
			...
			if (!strcmp(temp_seg->segment + temp_seg->offsets[m], segbase)
				...
				if (temp_seg->tid == tid)// && temp_seg->trans_begun!=1)
				{
					...
					if (duplicate == 0) {
						...
						temp_seg->offsets[temp_seg->offsetcount]
								= (long) offset;
						temp_seg->offsetcount++;
					...
					for (z = 0; z < temp_seg->offsetcount; z++) {
						...
						fprintf(logfile,
								temp_seg->segment + temp_seg->offsets[z]);
						fprintf(logfile, "@");
						//fprintf(logfile,"%");
					}
					fclose(logfile);
...
void rvm_commit_trans(trans_t tid) //commit all changes that have been made within the specified transaction. When the call returns, then enough information should have been saved to disk so that, even if the program crashes, the changes will be seen by the program when it restarts.
{
	...
	while (temp_seg != NULL) {
		...
		if (temp_seg->tid == tid && temp_seg->being_modified == 1
				&& temp_seg->tidcount == 0)// && temp_seg->mapped == 1)
		{
