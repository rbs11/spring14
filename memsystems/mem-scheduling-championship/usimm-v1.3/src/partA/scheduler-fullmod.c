	...
	if(drain_writes[channel])
	{
		...
		LL_FOREACH(write_queue_head[channel], wr_ptr)
		{
			if(wr_ptr->command_issuable)
			{
				...
				if (wr_ptr->next_command == PRE_CMD) {
					...
					int has_hit = 0;
					LL_FOREACH(write_queue_head[channel], curr_ptr) {
						if (curr_ptr->dram_addr.rank == wr_ptr->dram_addr.rank && curr_ptr->dram_addr.bank == wr_ptr->dram_addr.bank && curr_ptr->next_command == COL_WRITE_CMD) {
					 has_hit = 1;
					 break;
						}
					}
            ...
            LL_FOREACH(read_queue_head[channel],rd_ptr) {
            	if (rd_ptr->command_issuable) {
            		if (rd_ptr->next_command == COL_READ_CMD) {
      				  ...
      				  /*For this out of order read, check for similar requests in the
      				   * queue. If not found, allow this row to be force closed*/
      				  LL_FOREACH(read_queue_head[channel], curr_ptr) {
						if (!curr_ptr->request_served
	   ...
	   if (!write_issued && read_issued) {
		...
		if (cas_issued_current_cycle[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank]) {
			...
			if (wr_ptr->next_command == COL_READ_CMD)
				return;
			// no hit pending, auto precharge
			if (issue_autoprecharge(channel, wr_ptr->dram_addr.rank, wr_ptr->dram_addr.bank)) {
				recent_colacc[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = 0;
			}
			/* If a command hasn't yet been issued to this channel in this cycle, issue a precharge. !command_issued_current_cycle[channel]*/
			if (1) {
		...
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->command_issuable)
			{
				...
				if (rd_ptr->next_command == PRE_CMD) {
                  ...
                  int has_hit = 0;
                  LL_FOREACH(read_queue_head[channel], curr_ptr) {
                      if (curr_ptr->dram_addr.rank == rd_ptr->dram_addr.rank && curr_ptr->dram_addr.bank == curr_ptr->dram_addr.bank && curr_ptr->next_command == COL_READ_CMD) {
                          has_hit = 1;
                          break;
                      }
                  }
		...
		if (!read_issued) {
			LL_FOREACH(write_queue_head[channel],wr_ptr) {
				if (wr_ptr->command_issuable) {
					if (wr_ptr->next_command == COL_WRITE_CMD) {
						recent_colacc[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = 1;
						/*Check for similar requests, if not found force close the row*/
						LL_FOREACH(read_queue_head[channel], curr_ptr) {
						if (!curr_ptr->request_served
							&&	curr_ptr->dram_addr.rank == wr_ptr->dram_addr.rank
							&& curr_ptr->dram_addr.bank == wr_ptr->dram_addr.bank
